...
const siteConfigPatchSchema = z.object({
  ...
  learningAssistantTiers: z.object({
    low: z.object({ model: z.string().trim().min(1).max(200), reasoningEffort: z.enum(["low", "medium", "high", "xhigh", "max"]), pointMultiplier: z.number().min(0.1).max(20), freeInUnlimited: z.boolean() }),
    high: z.object({ model: z.string().trim().min(1).max(200), reasoningEffort: z.enum(["low", "medium", "high", "xhigh", "max"]), pointMultiplier: z.number().min(0.1).max(20), freeInUnlimited: z.boolean() }),
    max: z.object({ model: z.string().trim().min(1).max(200), reasoningEffort: z.enum(["low", "medium", "high", "xhigh", "max"]), pointMultiplier: z.number().min(0.1).max(20), freeInUnlimited: z.boolean() }),
  }).optional(),
  ...
  qqGroupAdReviewPeakServiceId: z.string().trim().max(48).optional(),
...
adminRouter.patch("/site-config", adminOnly, validate(siteConfigPatchSchema), async (req, res, next) => {
  try {
    if (
      ...
      req.body.qqGroupAdReviewPeakServiceId !== undefined ||
